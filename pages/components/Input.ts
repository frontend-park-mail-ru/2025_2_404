...
export default class Input {
  ...
  validate(value: string): string | null {
    ...
    const inputEl = document.getElementById(this.id) as HTMLInputElement | null;
    if (!inputEl) {
      return errorMessage;
    }
  ...
  PasswordToggle(): void {
    const inputWrapper = document.getElementById(this.id)?.parentElement;
    const toggle = inputWrapper?.querySelector('.password-toggle') as HTMLElement | null;
    if (toggle) {
